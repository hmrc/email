...
class RawEventSpec extends SpecBase {

  import RawEvent.reads

  "Json Reads" should {
    "read the json correctly" in new Setup {
      Json.parse(rawEventJsonString1).as[RawEvent] mustBe rawEvent
    }

    "throw the exception for invalid json" in new Setup {
      intercept[JsResultException] {
        Json.parse(rawEventInvalidJsonString).as[RawEvent]
      }
    }
  }
  ...
  "Json Writes" should {
    "write the object correctly" in new Setup {
      Json.toJson(rawEvent) mustBe Json.parse(rawEventJsonString)
    }
  }
  ...
  trait Setup {
    ...
    val rawEventJsonString1: String =
      """{
        |"deliveryInfoNotification":{
        |"deliveryInfo":{
        |"timeStamp":"2025-12-06T11:30:50",
    ...
    val rawEventInvalidJsonString: String =
      """{
        |"deliveryInfoNotification":{
        |"deliveryInfo":{
        |"timeStamp":"2025-12-06T11:30:50",
